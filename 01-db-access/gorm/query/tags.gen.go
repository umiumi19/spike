...
func newTag(db *gorm.DB, opts ...gen.DOOption) tag {
	...
	_tag.Posts = tagManyToManyPosts{
		...
		Author: struct {
			...
			RelationField: field.NewRelation("Posts.Author", "model.User"),
			Posts: struct {
				field.RelationField
			}{
				RelationField: field.NewRelation("Posts.Author.Posts", "model.Post"),
			},
		},
		Comments: struct {
			field.RelationField
...
func (t tag) clone(db *gorm.DB) tag {
	...
	t.Posts.db = db.Session(&gorm.Session{Initialized: true})
	t.Posts.db.Statement.ConnPool = db.Statement.ConnPool
	return t
}

func (t tag) replaceDB(db *gorm.DB) tag {
	t.tagDo.ReplaceDB(db)
	t.Posts.db = db.Session(&gorm.Session{})
	return t
}
...
func (a tagManyToManyPosts) Unscoped() *tagManyToManyPosts {
	a.db = a.db.Unscoped()
	return &a
}
...
func (a tagManyToManyPostsTx) Unscoped() *tagManyToManyPostsTx {
	a.tx = a.tx.Unscoped()
	return &a
}
